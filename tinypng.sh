...
# 获取最新提交的新增图片文件
last_add_img_files=$(git show --pretty="" --name-status HEAD | grep '^A(jpg\|png\|gif\|ico)' | awk '{print $2}')
...
last_update_img_files=$(git show --pretty="" --name-status HEAD | grep '^M(jpg\|png\|gif\|ico)' | awk '{print $2}')
